...
#+RESULTS:
: 37
And here's a little helper that we'll use below to parse the 6.txt file:
#+BEGIN_SRC lisp
  (defun challenge-6-bytes (pathspec)
...
#+RESULTS:
: CHALLENGE-6-BYTES
Next, we attempt to find the key size of the XOR'd challenge text. My
...
#+BEGIN_SRC lisp
  (loop with bytes = (challenge-6-bytes "6.txt")
        for i from 1 to 20
...
#+BEGIN_SRC lisp
  (let ((bytes (challenge-6-bytes "6.txt")))
    (cp:bytes->ascii
