...
#+RESULTS:
: ((T . 3) (T . 3) (T . 3) (NIL) (T . 3) (T . 3) (NIL) (T . 3) (NIL) (NIL))
: T
** Byte-at-a-time ECB decryption (Simple)
*** Detect the block size
#+BEGIN_SRC lisp
  (let ((oracle (cp:make-oracle)))
    (loop for i from 1 to 64
          for encrypted = (funcall oracle
                                   (make-array (* i 2) :initial-element 97))
...
#+BEGIN_SRC lisp
  (let ((oracle (cp:make-oracle)))
    (cp:detect-aes-128-ecb
     (funcall oracle (make-array (* 16 2) :initial-element 97))))
#+END_SRC
...
: "
**** TODO Profile this, it's super slow
My guess is that most of the time is eaten by all the decryptions,
but I can probably save memory use by getting smarter about my hash
table keys (right now they're blocks as hex strings), and by taking
less subsequences.
